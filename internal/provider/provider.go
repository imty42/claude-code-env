...
type ProviderManager struct {
	...
	robinIndex      int
	lastSelected    string // 上次选择的 provider 名称
	mutex           sync.RWMutex
}
...
func (pm *ProviderManager) getNextDefault(availableProviders []*ProviderState) *ProviderState {
	...
	for _, ps := range pm.providers {
		for _, available := range availableProviders {
			if ps == available {
				// 只在 provider 切换时记录日志
				if pm.lastSelected != ps.Provider.Name {
					prevProvider := pm.lastSelected
					if prevProvider == "" {
						prevProvider = "<无>"
					}
					logger.Debug(logger.ModuleProvider, "切换 provider: %s -> %s (default策略)", prevProvider, ps.Provider.Name)
					pm.lastSelected = ps.Provider.Name
				}
				return ps
			}
		}
...
func (pm *ProviderManager) getNextRobin(availableProviders []*ProviderState) *ProviderState {
	if len(availableProviders) == 1 {
		// 只在 provider 切换时记录日志
		if pm.lastSelected != availableProviders[0].Provider.Name {
			prevProvider := pm.lastSelected
			if prevProvider == "" {
				prevProvider = "<无>"
			}
			logger.Debug(logger.ModuleProvider, "切换 provider: %s -> %s (robin策略，仅一个可用)", prevProvider, availableProviders[0].Provider.Name)
			pm.lastSelected = availableProviders[0].Provider.Name
		}
		return availableProviders[0]
	}
	...
	if pm.lastSelected != selected.Provider.Name {
		...
		logger.Debug(logger.ModuleProvider, "切换 provider: %s -> %s (robin策略，索引: %d)", prevProvider, selected.Provider.Name, pm.robinIndex-1)
		pm.lastSelected = selected.Provider.Name
	}
	return selected
}
